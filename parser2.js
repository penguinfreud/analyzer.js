...
var Lexer = extend(Object, {
	...
	space: function () {
		...
		if (ch === 32) ch = this.text.charCodeAt(++this.pos);
		if (ch <= 32 || ch >= 160 || ch === 47) this._space(ch);
	},
...
var Parser = extend(Object, {
	...
	next: function (forceRegExp) {
		var t = this.buffer.shift() || this.lexer.next(forceRegExp);
		this.last = t.end;
		return t;
	},
	
	advance: function () {
		this.last = this.buffer.shift().end;
	},
	...
	match: function (code) {
		var t = this.buffer[0];
		if (!t) t = this.buffer[0] = this.lexer.next();
		if (t.type === TOKEN_PUNC && t.value === code) {
			this.advance();
			return t;
		}
		return 0;
	...
	peekMatch: function (code, depth) {
		var t = this.peek(false, depth);
		return t.type === TOKEN_PUNC && t.value === code;
	},
	
	ident: function () {
		var t = this.buffer[0];
		if (!t) t = this.buffer[0] = this.lexer.next(true);
		if (t.type === TOKEN_ID) {
			this.advance();
			return t.value;
		}
	...
	pos: function () {
		return this.buffer.length > 0? this.buffer[0].start: this.lexer.pos;
	}
});

function semicolon(p) {
	if (p.match(59)) return 1;
	var t = p.peek();
	if (t.type === TOKEN_PUNC && t.value === 125 || t.hasNewLine || p.finished())
		return 1;
	return 0;
}
...
function Statement(p) {
	...
	if (t.type === TOKEN_ID) {
		if (_stmtMapKeys[t.value] === 1) {
			p.advance();
			return _stmtMap[t.value](p, pos);
		}
		if (p.peekMatch(58, 2)) return LabelStmt(p, pos);
	...
	return ExprStmt(p, pos);
}

function FunctionExpr(p, pos) {
	var name = p.ident() || "";
	var func = FunctionLiteral(p, pos);
	func.name = name;
	return func;
}

function FunctionDecl(p, pos) {
	var name = p.expectIdent();
	var func = FunctionLiteral(p, pos);
	func.name = name;
	return func;
}
...
function ExprStmt (p,pos) {
	var e=AssignExpr(p,0),l;
	if (!p.match(44)) {
		semicolon(p);
		return e;
	}
	l=[e];
	do l.push(AssignExpr(p,0));
	while(p.match(44));
	semicolon(p);
	return p.make(EXPR_LIST,pos,{list:l});
}
...
function Expression (p, noIn) {
	var pos=p.pos(),e=AssignExpr(p,noIn),l;
	if (!p.match(44))return e;
	l=[e];
	do l.push(AssignExpr(p,noIn));
	while(p.match(44));
	return p.make(EXPR_LIST,pos,{list:l});
}
