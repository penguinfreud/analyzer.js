var parser = (function () {
            ...
            if (hasId || parseIdentifier()) {
                if (state) {
                    ...
                    if (keywordsAndReservedWordsAndBadIds[value] !== 1)
                        ...
                        expr = FuncRef(parseFunctionWithName(0));
                    } else if (value === "null")
                        expr = Constant(null);
                    else if (value === "false")
                        expr = Constant(false);
                    ...
                    if (precedence === 12) {
                        ...
                        while (prevPrecedence <= precedence) {
                            if (prevOp === 38) {
                                ...
                                expr = CompoundAssign(prevOp, prevExpr, expr);
                            } else {
                                expr = Binary(prevOp, prevExpr, expr);
                            }
    ...
    var parseSwitch, parseTry, parseWith;
    ...
    return {
        ...
        parseFunc: function ($text) {
            ...
            hasLineTerminator = 0;
            space();
            var func = parseFunctionWithName(0);
            restore();
            return func;
        }
    }
