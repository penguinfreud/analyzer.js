...
var AbstractScope = extend(ScopeBase, {
    varMap: null,
    ...
    resolveRef: function (ref) {
        ...
        if (typeof ref.attr === "string") {
            var v, name = ref.attr;
            if (hasOwnProperty.call(this.varMap, name)) {
                ref.attr = this.varMap[name];
            } else {
                v = this.localVar();
                v.name = name;
                ref.attr = this.varMap[name] = v;
            }
        }
    }
});
...
var FuncBase = extend(Scope, {
    ...
    __init__: function () {
        ...
        this.head.attr = this;
        this.body = new Node(FUNC);
        this.body.attr = this;
    },
    
    setBody: function (body) {
        addChild(this.body, body);
        restoreScope();
        restoreScope();
    },
...
var FuncRef = function (func) {
    var node = new Node(FUNC_REF);
    node.attr = func;
    return node;
};

var Param = function (_var, func) {
...
var getDominator = function (node, stack, reverse) {
    ...
    if (!reverse && (node.flags & isLoopHead)) {
        ...
        t = preds[0];
        for (i = 1; i<l; i++) {
            t = _commonDom(t, preds[i], stack, reverse);
        }
        if (s) {
            stack.pop();
        }
...
var CommonDom = function (a, b, reverse) {
    return _commonDom(a, b, [], reverse);
};
...
var GetDefinition = function (node, ref) {
    ...
    if (ref.type === REFERENCE) {
        ...
        if (type === ASSIGN) {
            ...
            if (!rhs) {
                if (scope.funcs.indexOf(_var) >= 0) {
                    rhs = FuncRef(ref.attr);
                } else if (scope.locals.indexOf(_var) >= 0) {
                    rhs = Constant(undefined);
                } else {
                    rhs = Param(_var, node.attr);
                }
...
var Equiv = function (a, b) {
    ...
    if (type === b.type) {
        if (type === REFERENCE) {
            ...
            return aa === ba || isNaN(aa) && isNaN(ba);
        } else if (type === NATIVE) {
            return aa === ba;
        } else if (type === UNARY) {
            return aa === ba &&
            ...
            return Equiv(ac[ac.length - 1], bc[bc.length - 1]);
        } else {
            l = ac.length;
            if (l !== bc.length) {
                return 0;
...
var _ControlDep = function (node) {
    ...
    while (pdom) {
        for (i = 0; i<l; i++) {
            pred = preds[i];
            if (pdom != pred && !postDominate(pdom, pred) && dominate(pred, pdom)) {
                if (pred.succ[0] === node) {
                    pdom.controlDep = {
                        node: pred,
                        neg: 0
...
var ConstProp = function (node) {
    ...
    if (type === UNARY && attr !== 6) {
        ...
        if (!isLHS(node)) {
            ...
            if (b.type === CONSTANT) {
                bv = b.attr;
                if (a.type === CONSTANT || a.type === NATIVE) {
...
var dump = function (node, visited) {
    ...
    console.log(
        ...
        ",\tcontrolDep: " +
            (node.controlDep && ((node.controlDep.neg? "!": "") +
                node.controlDep.node))
    );
